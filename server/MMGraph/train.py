...
def one_task(name,Kmers,adjs_0, adjs_1,adjs_3, idx_train, idx_val, idx_test, idx_all, labels, options, save=False):
    n = len(labels)
    ...
    def evaluate(feature0, feature1, feature3, support0, support1, label, mask, placeholders):
        feed_dict_val = construct_feed_dict(
            feature0, feature1,feature3, support0, support1, label, mask, placeholders)
    ...
    cost_val=[]
    # train model
    feed_dict = construct_feed_dict(feature0, feature1, feature3, support0, support1, labels, train_mask, placeholders)
    ...
    for epoch in tqdm(range(options['epochs']+1)):
        ...
        if epoch % 2 == 0:
            val_loss, preds, labels,_,_= evaluate(feature0, feature1,feature3, support0, support1, labels, val_mask, placeholders)
            # print(preds)
        ...
        if epoch == options['epochs']:
            test_loss, preds, labels,seq_hidden, kmer_hidden= evaluate(feature0, feature1,feature3, support0, support1, labels, test_mask, placeholders)
